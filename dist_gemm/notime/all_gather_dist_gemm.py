...
if __name__ == "__main__":
    ...
    print(f"Rank {rank}: local_A shape = {local_A.shape}, local_C shape = {local_C.shape}")
    ...
    if rank == 0:
        ...
        atol = 1e-8
        rtol = 1e-6
        max_abs_error = torch.max(torch.abs(global_C - expected_C))
        max_rel_error = torch.max(torch.abs((global_C - expected_C) / expected_C))

        if torch.allclose(global_C, expected_C, atol=atol, rtol=rtol):
