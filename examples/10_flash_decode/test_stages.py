...
def _iris_split_kv_kernel_stable(
    ...
    offs_out = (
        bid * stride_intermediate_bs + hid * stride_intermediate_h + split_kv_id * stride_intermediate_split + offs_d
    )
    tl.store(intermediate_out_ptr + offs_out, acc, mask=mask_d)
    offs_log = (
        bid * stride_intermediate_bs
        + hid * stride_intermediate_h
...
def test_stage_A(shmem):
    ...
    intermediate_out = torch.empty(
        (batch_size, total_q_heads, num_kv_splits, head_dim + 1), dtype=torch.float32, device="cuda"
    )
    ...
    _iris_combine_local_kv_kernel[grid2](
        intermediate_out,
        final_local_out,
        kv_lens,  # We repurpose the combine kernel to get the final normalized output
        intermediate_out.stride(0),
        intermediate_out.stride(1),
        intermediate_out.stride(2),
        final_local_out.stride(0),
        final_local_out.stride(1),
        total_q_heads,
        head_dim,
        num_kv_splits,
    )
