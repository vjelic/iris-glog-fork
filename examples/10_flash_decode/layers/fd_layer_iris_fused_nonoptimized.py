...
def gqa_local_kernels_fused(
    ...
    # print(f"{kk1.n_regs} registers used first, {kk1.n_spills} spills")
    ...
    # print(f"{kk2.n_regs} registers used second, {kk2.n_spills} spills")

class SpGQAFlashDecodeAttentionIrisFused(torch.nn.Module):
    def __init__(self, iris_instance, rank, node, num_ranks, num_nodes, num_q_heads, num_kv_heads, q_head_dim, v_head_dim, page_size=1,
        ...
        self.signal_flags = self.iris_instance.zeros(
            (self.num_ranks, self.num_ranks), dtype=torch.int32
        )
