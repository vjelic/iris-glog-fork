...
def _broadcast_kernel(
    data_ptr, n_elements, heap_bases_ptr, root_rank: tl.constexpr, cur_rank: tl.constexpr, BLOCK_SIZE: tl.constexpr
):
...
def _iris_split_kv_kernel(
    ...
    stride_v_cache_bs,
    stride_v_cache_h,
    stride_intermediate_bs,
    stride_intermediate_h,
    stride_intermediate_split,
    stride_table_bs,
    ...
    for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
        ...
        k = tl.load(
            k_ptr + offs_cache_k, mask=(offs_n[None, :] < split_kv_end) & (offs_d[:, None] < BLOCK_HEAD_DIM), other=0.0
        )
        ...
        e_max = n_e_max
    offs_out = (
        bid * stride_intermediate_bs
        + cur_head[:, None] * stride_intermediate_h
        + split_kv_id * stride_intermediate_split
        + offs_d[None, :]
    )
    tl.store(
        intermediate_out_ptr + offs_out, acc / e_sum[:, None], mask=mask_h[:, None] & (offs_d[None, :] < BLOCK_HEAD_DIM)
    )
    offs_log = (
        bid * stride_intermediate_bs
        + cur_head * stride_intermediate_h
        + split_kv_id * stride_intermediate_split
        + BLOCK_HEAD_DIM
    )
...
def _iris_combine_local_kv_kernel(
    ...
    for split_kv_id in range(0, NUM_KV_SPLITS):
        if (split_kv_id * tl.cdiv(cur_kv_seq_len, NUM_KV_SPLITS)) < cur_kv_seq_len:
            tv = tl.load(
                intermediate_out_ptr + offs_v + split_kv_id * stride_intermediate_split,
                mask=offs_d < BLOCK_HEAD_DIM,
                other=0.0,
            )
...
def _iris_combine_global_kv_kernel(
    ...
    for rank_id in range(num_ranks):
        ...
        offs_logic = (
            rank_id * stride_gathered_rank + bid * stride_gathered_bs + hid * stride_gathered_h + BLOCK_HEAD_DIM
        )
...
class SpGQAFlashDecodeIris(torch.nn.Module):
    def __init__(self, shmem, num_q_heads, num_kv_heads, head_dim, batch_size, page_size=1):
        ...
        self.intermediate_out = self.shmem.empty(
            (batch_size, self.num_q_heads, self.num_kv_splits, self.head_dim + 1), dtype=torch.float32
        )
        ...
        self.gathered_buffer = self.shmem.empty(
            (self.num_ranks, batch_size, self.num_q_heads, self.head_dim + 1), dtype=torch.float32
        )
    ...
    def forward(self, q, k_cache, v_cache, block_table, kv_lens_per_rank):
        ...
        _iris_split_kv_kernel[grid_stage1](
            ...
            v_cache.stride(0),
            v_cache.stride(2),
            self.intermediate_out.stride(0),
            self.intermediate_out.stride(1),
            self.intermediate_out.stride(2),
        ...
        _iris_combine_local_kv_kernel[grid_stage2](
            ...
            self.intermediate_out.stride(1),
            self.intermediate_out.stride(2),
            self.local_out.stride(0),
            self.local_out.stride(1),
            self.num_q_heads,
            self.head_dim,
            self.num_kv_splits,
        )
        ...
        _iris_combine_global_kv_kernel[grid_stage3](
            ...
            self.final_output,
            self.gathered_buffer.stride(0),
            self.gathered_buffer.stride(1),
            self.gathered_buffer.stride(2),
            self.final_output.stride(0),
            self.final_output.stride(1),
            self.num_q_heads,
            self.num_ranks,
            self.head_dim,
        )
...
def final_ref_paged_attn(
    ...
    block_tables_global = torch.cat(
        [bt_shard + i * num_pages_per_rank for i, bt_shard in enumerate(block_tables_all_shards)], dim=1
    )
...
if __name__ == "__main__":
    ...
    sp_gqa_attn = SpGQAFlashDecodeIris(shmem, total_q_heads, total_kv_heads, head_dim, batch_size, page_size=page_size)
    shmem.barrier()

    if rank == 0:
        print(f"[RANK {rank}] --- Creating mock data ---")
    q = shmem.empty((batch_size, total_q_heads, head_dim), dtype=torch.float16)
    if rank == 0:
        q.copy_(torch.randn(batch_size, total_q_heads, head_dim, device="cuda", dtype=torch.float16))

    shmem.barrier()
    if rank == 0:
        print(f"[RANK {rank}] Broadcasting Q tensor to all ranks...")
    _broadcast_kernel[(triton.cdiv(q.numel(), 1024),)](q, q.numel(), shmem.get_heap_bases(), 0, rank, 1024)
    shmem.barrier()

    num_pages_per_rank = (seq_len_per_rank + page_size - 1) // page_size
    k_cache_shard = shmem.randn((num_pages_per_rank, page_size, kv_heads_per_rank, head_dim), dtype=torch.float16)
    v_cache_shard = shmem.randn((num_pages_per_rank, page_size, kv_heads_per_rank, head_dim), dtype=torch.float16)
    ...
    if rank == 0:
        print(f"[RANK {rank}] --- Gathering data on all ranks for correctness check ---")
    ...
    _iris_all_gather_kernel[(triton.cdiv(k_cache_shard.numel(), 1024),)](
        k_cache_shard, k_list_flat, shmem.get_heap_bases(), k_cache_shard.numel(), rank, world_size, 1024
    )
    _iris_all_gather_kernel[(triton.cdiv(v_cache_shard.numel(), 1024),)](
        v_cache_shard, v_list_flat, shmem.get_heap_bases(), v_cache_shard.numel(), rank, world_size, 1024
    )
    _iris_all_gather_kernel[(triton.cdiv(block_table_shard.numel(), 1024),)](
        block_table_shard, bt_list_flat, shmem.get_heap_bases(), block_table_shard.numel(), rank, world_size, 1024
    )
    shmem.barrier()

    if rank == 0:
        print(f"[RANK {rank}] --- Data gathering complete ---")
        ...
        # Call the new, corrected reference function
        ...
        try:
            torch.testing.assert_close(output_single, ref_output, atol=1e-1, rtol=1e-2)
            print("\n✅ Correctness Test Passed!")
        except AssertionError as e:
            print("\n❌ Correctness Test Failed!")
            print(f"Max absolute difference: {torch.max(torch.abs(output[0] - ref_output))}")
