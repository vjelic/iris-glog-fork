...
# Import the Iris library
import iris

# Assuming decode_kernels are local or in the path
...
class SpGQAFlashDecodeAttention(torch.nn.Module):
    ...
    def __init__(self, rank, node, num_ranks, num_nodes, num_q_heads, num_kv_heads, q_head_dim, v_head_dim, page_size=1,
        ...
        # Initialize Iris (this should be done once per process)
        ...
        # Pre-allocate a buffer on the Iris symmetric heap for a *conceptual* allgather output.
        # Note: The actual `mpi_allgather` used later handles its own communication buffers.
    ...
    def forward(self, q, k_cache, v_cache, global_kv_lens, block_table):
        ...
        # CRITICAL FIX: Convert to float32 BEFORE passing to mpi_allgather
        output_combine_numpy_flattened = output_combine.cpu().numpy().astype(np.float32).flatten()

        # Perform the allgather operation using Iris's MPI helper
        ...
        # Use the original desired dtype for the Torch tensor (q.dtype, which is float16)
        ...
        ).to(q.dtype) # Convert back to original dtype if needed, otherwise keep float32
