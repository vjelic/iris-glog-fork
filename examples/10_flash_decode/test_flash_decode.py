...
def kernel_gqa_fwd_batch_decode_split_kv(
    ...
    q_ptr, k_cache_ptr, v_cache_ptr, output_ptr, block_table_ptr, kv_length_ptr,
    # Scalar value
    sm_scale,
    # Strides for local tensors
    stride_q_bs, stride_q_h, stride_k_cache_bs, stride_k_cache_h,
    stride_v_cache_bs, stride_v_cache_h, stride_o_bs, stride_o_h, stride_o_split, stride_table_bs,
    # Kernel constants
    kv_group_num: tl.constexpr, q_head_num: tl.constexpr, BLOCK_HEAD_DIM: tl.constexpr,
    BLOCK_DPE: tl.constexpr, BLOCK_DV: tl.constexpr, BLOCK_N: tl.constexpr,
    BLOCK_H: tl.constexpr, NUM_KV_SPLITS: tl.constexpr, PAGE_SIZE: tl.constexpr,
    ...
    for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
        ...
        if BLOCK_DPE > 0:
            ...
            kpe = tl.load(k_cache_ptr + offs_cache_kpe, mask=(offs_n[None, :] < split_kv_end) & mask_dpe[:, None], other=0.0)
    ...
    for remote_rank in range(world_size):
        ...
        for split_kv_id in range(0, NUM_KV_SPLITS):
            if (tl.cdiv(cur_batch_seq_len, NUM_KV_SPLITS) * split_kv_id) < cur_batch_seq_len:
                ...
                tv = iris.load(Mid_O + offs_v + split_kv_id * stride_mid_os, cur_rank, remote_rank, heap_bases, mask=mask_d)
...
def gqa_fwd_batch_decode_iris(shmem, q, k_cache, v_cache, kv_lens, block_table, scale):
    ...
    kernel_gqa_fwd_batch_decode_split_kv[grid_split_kv](
        q, k_cache, v_cache, output_split, block_table, kv_lens,
        scale,
        q.stride(0), q.stride(1), k_cache.stride(-3), k_cache.stride(-2),
        v_cache.stride(-3), v_cache.stride(-2), output_split.stride(0),
        output_split.stride(1), output_split.stride(2), block_table.stride(0),
        kv_group_num=kv_group_num, q_head_num=q_heads, BLOCK_HEAD_DIM=BLOCK_HEAD_DIM,
        BLOCK_DPE=BLOCK_DPE, BLOCK_DV=BLOCK_DV, BLOCK_N=BLOCK_N, BLOCK_H=BLOCK_H,
        NUM_KV_SPLITS=NUM_KV_SPLITS, PAGE_SIZE=page_size,
        K_DIM=q_head_dim, V_DIM=v_head_dim
    )
    shmem.barrier()
    ...
    kernel_gqa_fwd_batch_decode_combine_kv[grid_combine_kv](
        output_split, final_output, kv_lens, shmem.get_heap_bases(),
        output_split.stride(0), output_split.stride(1), output_split.stride(2),
        final_output.stride(0), final_output.stride(1),
        NUM_KV_SPLITS=NUM_KV_SPLITS, BLOCK_DV=BLOCK_DV, Lv=v_head_dim,
        cur_rank=rank, world_size=shmem.get_num_ranks()
    )
    shmem.barrier()
...
def main():
    ...
    if rank == 0:
        ...
        if np.isfinite(output_local).all() and not np.all(output_local == 0):
            ...
            shmem.log(f"❌ Correctness check failed. Output contains NaNs/Infs or is all zeros.")
